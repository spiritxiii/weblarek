...
import { ApiPostMethods, IApi, IProduct, IBuyer, TPayment, IProducts, IOrderRequest, IOrderResponse } from './types/index';
...
function testCatalog() {
  try {
    ...
    if (firstProduct.id !== products[0].id) {
    ...
    if (firstProduct.id !== currentProduct.id) {
